...
class PotentialFieldNavigationNode(Node):
    def __init__(self):
        super().__init__('potentialfield_nav_node')

        self.cmd_pub = self.create_publisher(TwistStamped, 'cmd_vel', 10)
        self.forward_speed = 0.5 # constant forward velocity
        ...
        # Clicked point from map visualization tools/simulator UI.
        # Many tools use /clicked_point (geometry_msgs/PointStamped).
        self.clicked_point_sub = self.create_subscription(
            PointStamped, '/clicked_point', self.clicked_point_callback, 10
        )
        # Optional alternate topic some tools expose.
        self.goal_point_sub = self.create_subscription(
            PointStamped, 'goal_point', self.clicked_point_callback, 10
        )
        self.goal_x = 0.0
        self.goal_y = 0.0
        ...
        self.get_logger().info(
            'PotentialFieldNavigationNode started. Click a goal point in the map UI to begin.'
        )
    ...
    def clicked_point_callback(self, msg: PointStamped):
        """Store latest clicked goal point (map coordinates)."""
